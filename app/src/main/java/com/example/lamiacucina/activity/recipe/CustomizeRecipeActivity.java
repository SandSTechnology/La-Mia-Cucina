...
public class CustomizeRecipeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_customize_recipe);
        ...
        cookingSteps.setText(recipe.getInstruction());
        ...
        {
            Ingredient p = new Ingredient();
            p.setIngredientName(recipe.getIngredients().get(i).getIngredientName());
            p.setIngredientQuantity(recipe.getIngredients().get(i).getIngredientQuantity());
            p.setIngredientUnit(recipe.getIngredients().get(i).getIngredientUnit());
        ...
        SaveRecipe.setOnClickListener(view -> {
            if (filepath==null && Objects.equals(recipe.getImage(), ""))
            {
                Toast.makeText(CustomizeRecipeActivity.this, "No Image Selected", Toast.LENGTH_SHORT).show();
                return;
            }
            if (recipeName.getText() ==null || recipeName.getText().toString().equals(""))
            {
                Toast.makeText(CustomizeRecipeActivity.this, "Please type Recipe Name/Title", Toast.LENGTH_SHORT).show();
                recipeName.requestFocus();
                return;
            }
            if (al.size() == 0)
            {
                Toast.makeText(CustomizeRecipeActivity.this, "Please add some Ingredients", Toast.LENGTH_SHORT).show();
                return;
            }
    ...
    private void SaveRecipe(String ID,String title,String instructions,String ImgURL,ArrayList<Ingredient> arrayList) {
        ...
        firebaseDatabase.child("Recipes").child(ID).updateChildren(recipe);
    ...
    private void saveDatabase(String imgUrl) {
        ...
        SaveRecipe(recipe.getID(),recipeName.getText().toString(),cookingSteps.getText().toString(),imgUrl,al);
    }
